...
namespace Scenarios.OverrideObject
{
    ...
    public class Test : IRunnable
    {
        ...
        public void Run()
        {
            ...
            Assert.Equal(nameof(GetHashCode), recorder.Invocations[0].Invocation.Method.Name);

            avatar.ToString();
            Assert.Equal(2, recorder.Invocations.Count);
            Assert.Equal(nameof(ToString), recorder.Invocations[1].Invocation.Method.Name);

            avatar.Equals(new object());
            Assert.Equal(3, recorder.Invocations.Count);
            Assert.Equal(nameof(Equals), recorder.Invocations[2].Invocation.Method.Name);
        }
    }
}
